...
def leer_archivo(nombre_archivo: str) -> Optional[Problema]:
    ...
    p.grafo_distancias = [
        [0.0 for _ in range(p.num_nodos)] for _ in range(p.num_nodos)]
    ...
    if start_nodos > 0:
        ...
        for i in range(start_nodos, len(lineas)):
            ...
            try:
                partes = linea.split()
                nodo = Nodo(id=int(partes[0]), x=int(
                    partes[1]), y=int(partes[2]))
                p.nodos.append(nodo)
                nodos_leidos += 1
    ...
    if start_hubs > 0:
        ...
        for i in range(start_hubs, len(lineas)):
            ...
            try:
                partes = linea.split()
                hub = Hub(id_nodo=int(partes[0]),
                          costo_activacion=float(partes[1]))
                p.hubs.append(hub)
                hubs_leidos += 1
            except (ValueError, IndexError):
                pass
    ...
    if start_aristas > 0:
        for i in range(start_aristas, len(lineas)):
            ...
            try:
                partes = linea.split()
                if len(partes) >= 3:
                    u, v, peso = int(partes[0]), int(
                        partes[1]), float(partes[2])
...
def imprimir_problema(p: Problema) -> None:
    ...
    for hub in p.hubs:
        print(
            f"  Hub en Nodo {hub.id_nodo:2d}: Costo de Activación = {hub.costo_activacion:.2f}")

    print("\n--- PAQUETES ---")
    for paquete in p.paquetes:
        print(
            f"  Paquete {paquete.id:2d}: Origen={paquete.id_nodo_origen} -> Destino={paquete.id_nodo_destino}")
