...
def greedy_seed_mejor(matriz_distancias: List[List[float]],
                      deposito_id: int,
                      nodos_recarga: set,
    ...
    Greedy mejorado:
      ...
      3) Siempre fuerza el regreso al depósito si es alcanzable (misma convención que el anterior).
    """
    dem = demanda.copy()
    ...
    while restante > 0:
        if carga == 0:
            ...
            # --- lookahead a la recarga ---
            mejor_r, mejor_score = None, float('inf')
            for r in nodos_recarga:
                d_ur = matriz_distancias[u][r]
                if d_ur == float('inf'):
                    continue
                # min distancia desde r a algún destino con demanda>0
                mejor_min_rv = float('inf')
                for v, cnt in dem.items():
                    if cnt <= 0:
                        continue
                    dv = matriz_distancias[r][v]
                    if dv < mejor_min_rv:
                        mejor_min_rv = dv
                # si no hay destinos alcanzables desde r, saltar
            ...
            # si no encontramos recarga “buena”, fallback al más cercano como antes
            if mejor_r is None:
        ...
        # --- elegir el destino: más cercano, tie-break por demanda pendiente ---
        candidatos = [v for v, cnt in dem.items() if cnt > 0 and matriz_distancias[u][v] != float('inf')]
        if not candidatos:
            break

        # ordenar por (distancia asc, demanda desc)
    ...
    # forzar regreso al depósito si es alcanzable
    if ruta:
...
def resolver_opcion_a_backtracking(
    ...
    # Semilla greedy
    semilla = greedy_seed_mejor(matriz_distancias, deposito_id, nodos_recarga, demanda, capacidad_camion)
