...
def main():
    ...
    problema = leer_archivo("caso_grande.txt")
    if problema is None:
        sys.exit(1)
    ...
    # Ejecutar backtracking (podés setear forzar_regreso_al_deposito=True si querés)
    mejor = f.resolver_opcion_a_backtracking(
        ...
        forzar_regreso_al_deposito=True,  # ponelo True si el enunciado te exige volver
        max_llamadas_sin_mejora=None
    )

    # # Imprimir solución
    # print("\n=== MEJOR SOLUCIÓN (Opción A) ===")
    # print(f"Distancia total: {mejor.distancia:.2f}")
    # print("Ruta (nodos visitados):")
    # print(" -> ".join(map(str, mejor.ruta)))

    # (Opcional) Expandir cada salto con el camino real de Floyd
    print("\nRuta expandida con Floyd:")
    ruta_expandida = []
